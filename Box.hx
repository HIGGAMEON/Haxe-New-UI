...
class Box extends FlxSpriteGroup
{
	...
	public var tabs(default, null):Array<Tab> = [];
	
	public var selectedTab(default, set):Tab = null;
	...
	public function new(x:Float, y:Float, width:Int, height:Int, tabs:Array<String> = null)
	{
		...
		if(tabs != null)
		{
			for (tab in tabs)
			{
				var createdTab:Tab = new Tab(tab);
	...
	override function update(elapsed:Float)
	{
			...
			for (tab in tabs)
			{
				if(FlxG.mouse.overlaps(tab, camera))
				{
					...
					if(!_draggingBox && canMove && _pressedBox && FlxG.mouse.pressed && (Math.abs(FlxG.mouse.deltaScreenX) > 1 || Math.abs(FlxG.mouse.deltaScreenY) > 1))
					{
						...
						_draggingBox = true;
						if(broadcastBoxEvents) EventHandler.event(DRAG_EVENT, this);
					}
					...
					if(FlxG.mouse.justPressed)
					{
						...
						selectedTab = tab;
						_lastClick = 0;
						if(broadcastBoxEvents) EventHandler.event(CLICK_EVENT, this);
					}
		...
		if(_ignoreTabUpdate)
		{
			if(broadcastBoxEvents)
				EventHandler.event(MINIMIZE_EVENT, this);
		}
	...
	{
		var createdTab:Tab = new Tab(name);
	...
	private function set_selectedTab(v:Tab)
	{
		if(v != null)
		{
