...
class DropDownMenu extends InputText
{
	...
	public function new(x:Float, y:Float, list:Array<String>, callback:Int->String->Void)
	{
		...
		button = new FlxSprite(behindText.width + 1, 0).loadGraphic(BitmapData.fromImage('assets/embed/images/psych-ui/dropdown_button.png'), true, 20, 20);
	...
	{
		var lastFocus = InputText.focusOn;
		...
		if(FlxG.mouse.justPressed)
		{
			if(FlxG.mouse.overlaps(button, camera))
			{
				...
				if(lastFocus != this)
					InputText.focusOn = this;
				else if(InputText.focusOn == this)
					InputText.focusOn = null;
			}
		}
		...
		if(lastFocus != InputText.focusOn)
		{
			showDropDown(InputText.focusOn == this);
		}
		else if(InputText.focusOn == this)
		{
	...
	function clickedOn(num:Int, label:String)
	{
		...
		if(broadcastDropDownEvent) EventHandler.event(CLICK_EVENT, this);
	}
	...
	function addOption(option:String)
	{
		...
		var item:DropDownItem = cast recycle(DropDownItem);
		item.x = 1;
		item.y = 1;
		item.label = option;
