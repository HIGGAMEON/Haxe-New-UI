...
class RadioGroup extends FlxSpriteGroup
{
	...
	public var checkedRadio(default, set):RadioItem;
	...
	public function new(x:Float, y:Float, labels:Array<String>, space:Float = 25, maxItems:Int = 0, ?isHorizontal:Bool = false, ?textWidth:Int = 100)
	{
		...
		arrowUp = new FlxSprite().loadGraphic(BitmapData.fromImage('assets/embed/images/psych-ui/arrow_up.png'), true, 24, 18);
		arrowUp.animation.add('normal', [0]);
		arrowUp.animation.add('press', [1]);
		arrowUp.animation.play('normal');
		arrowUp.visible = false;
		arrowDown = new FlxSprite().loadGraphic(BitmapData.fromImage('assets/embed/images/psych-ui/arrow_down.png'), true, 24, 18);
...
class RadioItem extends CheckBox
{
	...
	{
		box.loadGraphic(BitmapData.fromImage('assets/embed/images/psych-ui/radio.png'), true, 16, 16);
